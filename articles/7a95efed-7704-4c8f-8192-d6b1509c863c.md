...
ですので、それぞれの環境変数を .env に書くにせよ .envrc に書くにせよ .envrc を設置してホストマシンにも読み込ませられる状態にしておくと
...
export DB_PORT=3306
export DB_NAME="my-app"
export DATABASE_URL="mysql://$DB_USERNAME:$DB_PASSWORD@$DB_HOST:$DB_PORT/$DB_NAME?$TEST"
```
...
if [ -f "./.envrc.secret" ]; then
  ...
  echo "ERROR: .env.secret が必要です。.env.personal.sample をコピーして作成してください" >&2
  exit 1
fi
